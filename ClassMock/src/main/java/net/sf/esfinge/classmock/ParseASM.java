...
class ParseASM {
    ...
    private void addClassDefaultConstructor() {
        ...
        if (!this.reader.isInterface()) {

            final MethodVisitor mv = this.cw.visitMethod(Opcodes.ACC_PUBLIC, "<init>", "()V", null, null);
            mv.visitCode();
    ...
    private void addMethods() {
        ...
        for (final IMethodReader method : this.reader.methods()) {
            ...
            } else if (Arrays.asList(int.class, short.class, byte.class, char.class, boolean.class).contains(method.returnType())) {
    ...
    private FieldVisitor toFieldVisitor(final IFieldReader field) {
        ...
        return this.cw.visitField(opcodes,
                        field.name(),
